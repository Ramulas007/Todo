...
import type { Card, Board, List as ListType } from "../../types/board.types";
...
type PendingMove = { cardId: string; targetListId: string } | null;
type BlockedMove = { cardId: string; reason: "done" | "overdue" | "backward" | "not-started" } | null;
...
export default function useCardMoveGuard(
	...
	function findListIdForCard(cardId: string) {
		return board.lists.find((l) => l.cardIds.includes(cardId))?.id;
	}
	...
	function attemptMove(cardId: string, sourceListId: string, targetListId: string) {
		...
		if (sourceListId === BACKLOG_LIST_ID && targetListId === IN_PROGRESS_LIST_ID && isCardAllPending(cardId)) {
	...
	function cancelCompletion() { setPendingCompletion(null); }
	function confirmBacklogMove() {
		if (!pendingConfirm) return;
		moveCard(pendingConfirm.cardId, pendingConfirm.targetListId);
		setPendingConfirm(null);
	}
	function cancelBacklogMove() { setPendingConfirm(null); }
	function dismissBlockedMove() { setBlockedMove(null); }

	return {
		handleDragEnd, pendingCompletion, pendingConfirm, blockedMove,
		confirmCompletion, cancelCompletion, confirmBacklogMove, cancelBacklogMove, dismissBlockedMove,
	};
}
